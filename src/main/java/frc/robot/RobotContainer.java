...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    if (usingXboxRemote){
      drivetrain.setDefaultCommand(xbxoArcadeDrive());
      new POVButton(xboxController, 0).onTrue(new POVDrive(drivetrain, "up"));           // Up
      new POVButton(xboxController, 45).onTrue(new POVDrive(drivetrain, "upright"));     // Up-Right
      new POVButton(xboxController, 90).onTrue(new POVDrive(drivetrain, "right"));       // Right
      new POVButton(xboxController, 135).onTrue(new POVDrive(drivetrain, "downright"));  // Down-Right
      new POVButton(xboxController, 180).onTrue(new POVDrive(drivetrain, "down"));       // Down
      new POVButton(xboxController, 225).onTrue(new POVDrive(drivetrain, "downleft"));   // Down-Left
      new POVButton(xboxController, 270).onTrue(new POVDrive(drivetrain, "left"));       // Left
      new POVButton(xboxController, 315).onTrue(new POVDrive(drivetrain, "upleft"));     // Up-Left
    } else {
